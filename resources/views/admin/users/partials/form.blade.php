...
@php
    // Normalizar variables para que el parcial funcione en create y edit
    /** @var \App\Models\User|null $user */
    $user = $user ?? null;

    // Acepta $dependencias o $deps (por compatibilidad)
    $dependencias = $dependencias ?? ($deps ?? collect());

    // Lista de roles por defecto si no se pasó desde el controlador
    $roles = $roles ?? ['Admin','Aprobador','Secretario','Usuario'];

    // Helper de valores (old() > modelo > default)
    $val = function ($field, $default = '') use ($user) {
        return old($field, $user?->{$field} ?? $default);
    };

    $isCreate = request()->routeIs('admin.users.create');
    $currentRole = old('role', $user ? ($user->getRoleNames()->first() ?? 'Usuario') : 'Usuario');
@endphp
